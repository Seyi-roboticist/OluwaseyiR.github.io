...
ASBRContext::vertex ASBRContext::sample_nearby(const ASBRContext::vertex& q, const ASBRContext::vertex& q_goal){
    ...
    static std::normal_distribution<> gaussian_generator(0, 1); // normal (gaussian) distribution
    static std::uniform_real_distribution<double> goal_random(0, 1);
    const double GOAL_BIAS{0.05};
    const double MAX_DEVIATION = M_PI / 75;

    // Check goal bias first and if not colliding, return goal directly
    ...
    do {
        for (size_t i = 0; i < q.size(); ++i) {
            q_rand[i] = q[i] + MAX_DEVIATION * gaussian_generator(gen);
            // Apply normalization or wrapping as needed here
        }
    ...
    // Fallback to the initial configuration if a non-colliding one isn't found
    return q;
}
...
//    if (state_collides(q) || state_collides(q_rand)) return false;
//
//    double distance = euclidean_norm(q, q_rand); // Assuming this function calculates Euclidean distance
//    double stepSize = std::min(0.03, 10 / distance); // Example adaptive step size logic
//    for (double t = stepSize; t <= 1.0; t += stepSize) { // Note <= to include endpoint
//        ASBRContext::vertex qt = interpolate(q, q_rand, t);
//        if (state_collides(qt)) return false;
//    }
//    return true;
//}
...
ASBRContext::path ASBRContext::search_path(const std::vector<vertex>& V,
    ...
    if (idx_init >= V.size() || idx_goal >= V.size()) {
        // Handle the error: either throw an exception, return an empty path, or handle it another way
        return P; // Returning an empty path for now
    }
    ...
    if (idx_init == idx_goal) {
        ...
        // Ensure idx_goal is within the range of parent before accessing it
        if (idx_goal >= parent.size()) {
            // Handle the error
            return P; // Returning an empty path for now
        }

        // Recursive case
        P = search_path(V, parent, idx_init, parent[idx_goal]);
...
ASBRContext::path ASBRContext::est(const ASBRContext::vertex& q_init, const ASBRContext::vertex& q_goal) {
    ...
    double nearThresholdVal = 30; // This seems excessively large, might need adjustment
    bool isPath = false;
